...
### Session du 12 août 2026 : ménage du dépôt + trois défauts structurels, en prod
Chantier en deux temps : nettoyage des branches, puis audit et correction. Poussé sur `main`.

- **Attribution GA4 réparée (`src/hooks/useAnalytics.js`)** : l'écouteur de clic délégué était en phase de **bulle**. React attache ses gestionnaires au conteneur racine, qui est dans le document : le `<Link>` naviguait donc AVANT, `history.pushState` était déjà appliqué, et `window.location.pathname` renvoyait la page d'arrivée. Tous les `cta_devis_click` partaient avec `page_path` = `/tarification`, effaçant la seule donnée utile de l'événement : quelle page déclenche l'intention. Passé en **phase de capture** (`addEventListener(..., true)`). **Règle : tout écouteur délégué au document qui lit l'URL courante doit être en capture, sinon il mesure la page d'arrivée.** Correctif repris de la branche `worktree-analytics-debugview` (PR #44, qui porte en plus un outil DebugView non repris : c'est une fonctionnalité, la PR reste ouverte).
- **Login d'auteur : trois formes, une seule normalisation.** `gh pr view --json author` passe par GraphQL, qui **préfixe** les applications (`app/dependabot`, `app/github-actions`), là où l'API REST **suffixe** (`dependabot[bot]`). Le portique testait `startsWith('dependabot')` et `gate.yml` comparait aux formes REST : l'exemption dependabot n'a jamais pu s'activer, et la PR #36 était rejetée en zone interdite depuis le 17 juillet. Le login est désormais normalisé **une seule fois** dans le job `contexte` (minuscules, `app/` et `[bot]` retirés) ; tout l'aval compare des logins nus. **Règle : ne jamais tester une forme particulière de login, toujours normaliser.**
- **Limite dure découverte : le `GITHUB_TOKEN` ne peut pas merger une PR qui touche `.github/workflows/`.** C'est le garde-fou GitHub contre un workflow qui se réécrit, et il n'existe **aucune** permission `workflows` attribuable à ce jeton : cela ne se contourne pas par configuration, seulement par un PAT ou une GitHub App portant le scope `workflow`. Or dependabot sur l'écosystème `github_actions` ne modifie que des fichiers de workflow : **ces PR ne peuvent jamais être auto-mergées**, elles demandent un merge manuel. Le cas est maintenant détecté avant le merge, commenté sur la PR, et les étapes suivantes (fermeture d'issue, rebuild, ping IndexNow) sont sautées puisque `main` n'a pas bougé.
- **Lien interne mort corrigé** : le crawl des 2180 liens internes en a trouvé un seul, vers une URL de l'ancien WordPress rattrapée par une 301. Repointé directement vers l'article CPI. **Règle : un lien interne ne dépend jamais d'une redirection.**
- **Dépôt** : 19 branches locales et 6 worktrees supprimés, 14 branches distantes nettoyées. Attention, piège vécu : le Gate merge en **squash**, donc `git branch --merged` ne voit pas les branches livrées ; et le test « la branche diffère-t-elle de main » est trompeur quand c'est `main` qui a avancé. Le seul test exact est `git merge-tree --write-tree main <branche>` puis comparaison des OID d'arbre par chemin, en ignorant `dist/`. Sept branches supprimées à tort ont dû être restaurées ainsi ; une (`claude/issue-2-20260710-2320`, première mission échouée, livrable déjà en prod via la PR #32) n'a pas pu l'être.
- **Non traité, décisions en attente** : `og:image` absent sur les 80 pages (nécessite un visuel 1200x630) ; 49 titles > 60 caractères et 23 meta descriptions > 155 (réécriture = arbitrage SEO d'Ayoub) ; **react-router 7.16.0 porte 3 CVE** (1 modérée, 2 hautes), corrigées en 7.18.2, montée non faite car le routeur est la colonne vertébrale du prérendu et l'historique du projet montre deux régressions d'hydratation (#418).

### Session du 13 juillet 2026 : carte grise, exactitude juridique + signaux GEO, en prod
Le site affirmait partout que **Certimat est « habilité par le Ministère de l'Intérieur »**. C'est faux, et c'était le principal risque juridique du site (pratique commerciale trompeuse, art. L.121-2 du Code de la consommation). Vérifié dans les CGU de Certimat (art. 1 et 2) : la société **EM Prestige Automobiles (EMPA)**, marque Certimat, agit comme **intermédiaire technologique**, n'est **pas** habilitée au SIV et **ne délivre aucun titre** ; ses CGU précisent même que le site « ne peut en aucun cas être assimilé ni confondu avec une administration publique ». La saisie dans le SIV est faite **exclusivement par un professionnel habilité** par le Ministère de l'Intérieur, mandaté directement par l'internaute, ou le dossier part à l'ANTS. Il n'existe donc **AUCUN numéro d'habilitation SIV ni agrément Trésor Public au nom de Certimat/EMPA** (absents de leurs mentions légales, vérifié).

- **Règle permanente** : ne JAMAIS écrire « Certimat est habilité », « service habilité », « partenaire agréé », ni afficher un numéro d'habilitation au nom de Certimat. Formulation verrouillée, à reprendre telle quelle : « Votre demande est traitée via Certimat, qui vous met en relation avec des professionnels habilités par le Ministère de l'Intérieur pour la saisie dans le SIV, ou transmise à l'Agence Nationale des Titres Sécurisés (ANTS / France Titres). » Le cadre est rappelé en tête de `src/pages/CarteGrise.jsx` et dans `src/assistant/knowledge.js` (le chatbot le disait aussi).
- **Nettoyage** : claim corrigé sur `/carte-grise` (hero, badges, FAQ, mentions), l'article `carte-grise-urgence-cpi-immediat`, l'article `combien-de-temps-carte-grise` (qui portait le pire : ligne de tableau attribuant l'habilitation préfectorale à Certimat), `assurer-sans-carte-grise`, `articlesData.js`, `tourSteps.js`, `knowledge.js`.
- **Délai chiffré (signal GEO n°1)** : les CGU de Certimat ne portent **qu'un seul délai contractuel**, le « traitement en 24h », défini comme **24 heures ouvrées** à réception d'un dossier complet. L'« édition immédiate » du CPI est annoncée sur leur site mais n'est **pas contractuelle** : on l'attribue explicitement à Certimat, jamais en promesse AssuTempo. D'où le chiffre tenu partout : **CPI sous 24 h ouvrées**. Une seule constante `CPI_DELAI` dans CarteGrise.jsx pilote hero, timeline, FAQ, meta et JSON-LD.
- **Sourcing YMYL corrigé** : le 750 € était attribué à service-public.fr, qui ne le porte pas. Chaîne exacte : délai d'1 mois = **article R322-5 du code de la route** ; ce qui est sanctionné = *maintenir le véhicule en circulation* sans carte grise à son nom (contravention de 4e classe, immobilisation possible) ; 135 € forfaitaire (90 € minorée, 375 € majorée) = service-public.gouv.fr ; **750 € = plafond des contraventions de 4e classe, article 131-13 du code pénal**. À noter : `service-public.fr` redirige en 301 vers **service-public.gouv.fr** (citations restantes à migrer dans les autres articles).
- **Ajouts GEO** (100 % dans le HTML prérendu, vérifié) : AnswerCapsule sur `/carte-grise` (le composant des articles, réutilisé), timeline chiffrée (T+0, 10 min, sous 24 h ouvrées, quelques jours ouvrés), carte « cadre légal » + carte « identité de l'opérateur » (EMPA SAS, SIRET 832 468 755 00018, RCS Versailles, siège Saint-Germain-en-Laye), FAQ portée à 8 questions, JSON-LD Service (avec `broker` = EMPA + SIRET, jamais `provider` : il n'est pas habilité) + HowTo + FAQPage, entrée `/carte-grise` dans `llms.txt`.
...
### Session du 14 septembre 2026 : l'usine produisait, c'est la publication qui était bloquée
Constat d'Ayoub : « le cycle de publication d'articles ne fonctionne plus ». Même illusion que le 1er septembre, cause différente. Le jeton est valide, le cron part bien les bons jours, et l'usine a écrit un article à chacun de ses quatre derniers runs. Trois défauts distincts, tous silencieux, corrigés (commits `2c48bcbe` et `a40832b5` sur `main`).

- **Plafond de tours trop bas sur les deux usines.** Les runs des 1, 3, 8 et 10 septembre ont consommé 90, 99, 90 et 89 tours pour un `--max-turns 80` : le job meurt en erreur **même quand le travail est terminé**. Trois fois sur quatre la PR avait eu le temps d'être ouverte (article publié, run rouge quand même), une fois non. La leçon avait déjà été tirée le 11 juillet sur `claude.yml` (200 tours) mais n'avait jamais été reportée sur `usine-contenu.yml` ni `chantier-structurel.yml`. **Règle : le plafond de tours n'est pas un garde-fou de coût, c'est une guillotine ; le vrai garde-fou est `timeout-minutes`.**
- **Un portique rouge ne prévenait personne.** La PR #60 du 10 septembre est restée ouverte et muette quatre jours pour un seul « En résumé » en titre de section. Le Gate n'écrivait sur la PR qu'en cas d'échec de *merge*, jamais en cas d'échec du *portique* : l'article existait, était bon, et n'était simplement jamais publié. `gate.yml` recopie désormais le rapport (fichier:ligne) en commentaire dès que le portique échoue. **Le rapport cite des extraits du diff, donc du texte potentiellement extérieur sur un dépôt public : il passe par `--body-file`, jamais par un heredoc ni une chaîne interprétée par le shell** (vérifié avec un rapport piégé contenant backtick, `$(...)` et `${...}` : recopiés littéralement).
- **Les usines ne connaissaient pas les règles qui les bloquent.** Leur prompt renvoyait à CLAUDE.md sans citer les expressions bannies ni les tirets interdits, et une session de 90 tours les oublie. Elles sont maintenant dans le prompt, et surtout **chaque usine lance `scripts/quality-gate.mjs` sur sa propre branche avant d'ouvrir la PR** : un article refusé est corrigé dans le run qui l'a écrit. **Piège à connaître : le portique analyse le DIFF COMMITÉ (`git diff origin/main...HEAD`), pas les fichiers du disque. Lancé avant le commit, il annonce « tout est vert » sans avoir rien lu.**
- **Journal d'anti-duplication non alimenté** : les runs des 8 et 10 septembre ont oublié la ligne de `.claude/usine-journal.md`, alors que le prompt la demandait. Elle devient une condition explicite du commit de livraison (idem `.claude/chantiers-state.md` pour les chantiers). Les deux lignes manquantes ont été rattrapées à la main.
- **Constaté, non corrigé (comportement GitHub)** : les crons de ce dépôt partent avec un retard systématique de 3 à 5 heures (usine planifiée à 06:17 UTC, exécutée vers 11:15 ; chantier à 16:23 UTC, exécuté vers 19:30). Le jour est respecté, le rythme deux fois par semaine aussi : ne pas « corriger » l'heure du cron, cela ne changerait rien.
- **Constaté aussi** : un `git push` sur la branche d'une PR ouverte n'a pas déclenché l'événement `synchronize` du Gate (attendu 6 min, rien, alors qu'un Gate dependabot passait au même moment). La porte de secours documentée fonctionne : `gh workflow run gate.yml -f pr_number=N`. À garder en tête avant de conclure à une panne du Gate.
- **Cause structurelle traitée : `dist/` est commité, donc tout chevauchement de deux PR produit un conflit garanti.** La PR #60 était `CONFLICTING` avec 241 fichiers en conflit, tous sous `dist/`, sources fusionnées sans une seule collision : le Chantier 3 avait été mergé entre-temps et les deux branches avaient rebuildé. `gh pr update-branch` ne sait pas résoudre cela. Le Gate fait maintenant un vrai `git merge origin/main` et **régénère `dist/` au lieu de le fusionner** (un artefact de build ne se fusionne pas), puis rejoue le portique avant de pousser. Il ne résout QUE si tous les conflits sont sous `dist/` : un seul conflit de source et il renonce, et toutes les étapes sont en `continue-on-error`, donc le comportement précédent (label `hold` + commentaire) reprend la main intact. Détection vérifiée dans les deux sens sur un dépôt jetable ; **le chemin complet en production n'a pas encore été exercé**, faute de PR réellement en conflit depuis le correctif.
- Écarté par les faits : l'échec de `claude-code-review` du 8 septembre était un incident transitoire d'installation du runner (`ENOENT ... /home/runner/.local/bin/claude`), pas une régression ; la revue du 11 septembre est repassée au vert.
...
- **Carte grise / Certimat** : Certimat (EM Prestige Automobiles) est un **intermédiaire technologique**, jamais « habilité par le Ministère de l'Intérieur », jamais « agréé ». La saisie SIV est faite par des professionnels habilités, ou le dossier part à l'ANTS. Aucun numéro d'habilitation au nom de Certimat, nulle part. Détail et formulation exacte : session du 13 juillet 2026, section 5.
- **Pages en marque blanche JL Assure (contractuel)** : `/tarification` porte l'iframe de souscription operee par JL Assure pour le compte du cabinet. Cette page ne doit afficher **aucun moyen de contact exterieur au parcours** : pas de numero du cabinet, pas de lien `tel:` ou `mailto:`, pas de WhatsApp, pas de formulation du type « appelez-nous » (demande ecrite du partenaire du 14 septembre 2026, applicable au 1er octobre 2026). Le client qui entre dans le tunnel ne doit avoir qu'un seul interlocuteur : son numero dedie s'affiche a l'interieur du tunnel. Formulation de remplacement en place : « Votre numero dedie s'affiche directement dans le tunnel de souscription, avec l'equipe qui suit votre dossier. » L'iframe **Certimat** de `/carte-grise` releve d'un autre partenaire et **n'est pas concernee** : le numero y reste affiche, comme sur tout le reste du site. Trois endroits a tenir synchrones pour toute nouvelle page concernee : `ROUTES_MARQUE_BLANCHE` dans `src/data/siteConfig.js`, la meme liste dans `api/chat.js` (fonction serverless CommonJS, elle ne peut pas importer le bundle client), et `PAGES_MARQUE_BLANCHE` dans `scripts/quality-gate.mjs` (regle 10, qui refuse toute PR reintroduisant une coordonnee ; controle fait sur le HTML servi, blocs `ld+json` exclus car le JSON-LD Organization porte legitimement telephone et email de l'editeur). L'assistant Tempo **reste disponible sur toutes les pages**, il renvoie simplement vers le numero du tunnel au lieu de donner celui du cabinet.
- **Animations** : transform et opacity uniquement (GPU), jamais width/height/top/left/margin. Aucun layout shift.
- **Drapeaux** : jamais d'emoji drapeau (paires U+1F1E6-U+1F1FF) dans le code ni le contenu. Windows n'a aucun glyphe de drapeau dans Segoe UI Emoji : il affiche les deux lettres du code pays en gris. Toujours `<Flag code="fr" />` (`src/components/ui/Flag.jsx`), qui sert les SVG de `public/flags`. Le champ de données est `code` (ISO 3166-1 alpha-2 minuscule), jamais un emoji. Un nouveau pays = déposer son SVG 4x3 dans `public/flags` et l'ajouter au Set `AVAILABLE` du composant. Les fichiers de `public/flags` sont servis en cache immuable (`vercel.json`) : si un dessin doit changer, renommer le fichier.
- **Fichier de contexte unique** : ce document est la seule source de vérité. `AGENTS.md` est un lien symbolique vers lui, pour les outils qui lisent ce nom par convention. Ne jamais en faire une copie : une copie de juillet avait déjà dérivé de six sessions et omettait la règle Certimat, la plus importante du projet. Idem pour les skills, qui vivent dans `.claude/skills` et nulle part ailleurs.
