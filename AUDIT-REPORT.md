...
2. ~~Pas de page 404 : soft-404 site-wide.~~ **Résolu le 6 juillet 2026** : page `NotFound.jsx` dans le style du site (accroche "Cette route ne mène nulle part", CTA devis + accueil + lien carte), route catch-all `*` partagée client/SSR, prérendu en `dist/404.html` (hors sitemap, noindex). La réécriture catch-all de `vercel.json` a été retirée : toutes les routes ayant leur fichier HTML physique, Vercel sert désormais `dist/404.html` avec un vrai statut 404 pour toute URL inconnue. Ne jamais remettre de rewrite vers `index.html` : il retransformerait chaque 404 en soft-404. QA : statuts vérifiés (200 pages réelles, 404 URL inconnues), hydratation propre, navigation depuis la 404 fonctionnelle, slug pays inconnu retombe en douceur sur la page Carte générique.
